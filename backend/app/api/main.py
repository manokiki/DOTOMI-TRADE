...
CORRECTIONS :
1. /recommendations/top : retourne état vide propre au lieu de 404
2. CORS : accepte toutes les origines Vercel + localhost
3. /health : retourne le vrai provider utilisé (Bybit)
"""
...
from app.db.models import (
    HumanCheckIn, SystemTradeSignal,
...
async def lifespan(app: FastAPI):
    ...
    await init_db()
    logger.info("startup_complete — provider=bybit+binance")
    ...
    if settings.scheduler_enabled:
        ...
        _scheduler_task  = asyncio.create_task(
            run_scan_loop(
                _market_registry, default_targets(),
    ...
    if _scheduler_task:
        try:
            ...
            pass
    if _market_registry:
        await _market_registry.aclose_all()
...
# CORS — CORRIGÉ : accepte Vercel + localhost
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://dotomi-trade.vercel.app",
        "https://*.vercel.app",
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        "http://localhost:3000",
...
async def health():
    ...
    return {
        ...
        "version":               "2.1.0",
        "market_data_provider":  "bybit (primary) + binance (fallback)",
        "market_data_reachable": ok,
...
async def health_markets():
    ...
    return {
        ...
        "markets": await _market_registry.health_check_all(),
    }
...
async def health_system(hours: int = 24, db: AsyncSession = Depends(get_db)):
    ...
    stmt  = (
        ...
        .where(SystemHealthLog.timestamp >= since)
        .order_by(desc(SystemHealthLog.timestamp))
    )
    ...
    for r in rows:
        c = by_c.setdefault(
            r.component,
            {"ok": 0, "down": 0, "last_status": None, "last_error": None},
        )
...
async def get_scanner(
    ...
    try:
        result = await scan_symbol(
            ...
            DEFAULT_USER_ID, settings.default_capital,
            state, db=db,
        )
...
async def top_recommendation(db: AsyncSession = Depends(get_db)):
    """
    CORRIGÉ : retourne un état vide propre au lieu de 404
    quand aucun setup n'est encore en base.
    """
    stmt = (
        select(TradeSetup)
        .where(TradeSetup.status == "AUTHORIZED")
        .order_by(desc(TradeSetup.total_score), desc(TradeSetup.created_at))
        .limit(3)
    )
    rows = (await db.execute(stmt)).scalars().all()

    if not rows:
        # Retourne un état vide — pas de 404
        return {
            "top": None,
            "alternatives": [],
            "message": "Aucun trade AUTORISÉ disponible. Lance un scan pour générer des setups.",
        }

    return {
        "top":          _ser_setup(rows[0]),
        "alternatives": [_ser_setup(r) for r in rows[1:]],
        "message":      None,
    }
...
async def compute_sizing(req: SizingRequest):
    try:
        r = compute_position_size(
            req.capital, req.entry_price, req.stop_loss,
...
async def risk_summary(
    ...
    t_cnt = (
        await db.execute(
            select(func.count(Trade.id)).where(func.date(Trade.opened_at) == today)
        )
...
async def capital_curve(db: AsyncSession = Depends(get_db)):
    total_pnl = (
        await db.execute(select(func.sum(Trade.pnl)).where(Trade.pnl.is_not(None)))
    ).scalar_one_or_none() or 0.0
    ...
    return {
        ...
        "progress_pct":    round(
            (capital_real - settings.default_capital) / (4000 - settings.default_capital) * 100, 1
        ),
    }
...
async def create_checkin(req: CheckInRequest, db: AsyncSession = Depends(get_db)):
    ...
    return {
        "id":              rec.id,
        "session_date":    today,
        "session_blocked": rec.session_blocked,
...
async def today_checkin(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "checkin_done":   True,
        "session_blocked": row.session_blocked,
        "block_reason":   row.block_reason,
        "fatigue":        row.fatigue,
        "stress":         row.stress,
        "fomo":           row.fomo,
        "revenge_mode":   row.revenge_mode,
...
async def validate_existing_setup(
    setup_id: int,
    db: AsyncSession = Depends(get_db),
):
    setup = (
        await db.execute(select(TradeSetup).where(TradeSetup.id == setup_id))
    ).scalar_one_or_none()
    if setup is None:
    ...
    sr = ScoreResult(
        ...
        sub_scores=SubScores(
            regime=setup.regime_score or 0,
            structure=setup.structure_score or 0,
            liquidity=setup.liquidity_score or 0,
            pullback=setup.pullback_score or 0,
            timing=setup.timing_score or 0,
            confirmation=setup.confirmation_score or 0,
            risk=setup.risk_score or 0,
            macro=setup.score_macro or 0,
            onchain=setup.score_onchain or 0,
        ),
        total_score=setup.total_score or 0,
        status=setup.status or "REJECTED",
        entry_price=setup.entry_price, stop_loss=setup.stop_loss,
        tp1=setup.tp1, tp2=setup.tp2,
        rrr=(
            abs(setup.tp1 - setup.entry_price) / abs(setup.entry_price - setup.stop_loss)
...
async def open_trade(
    ...
    checkin = (
        await db.execute(
            select(HumanCheckIn).where(HumanCheckIn.session_date == today).limit(1)
        )
    ).scalar_one_or_none()
    ...
    trade = Trade(
        ...
        macro_context=setup.macro_context,
...
async def close_trade(
    ...
    trade.r_multiple = (
        trade.pnl / (risk_unit * trade.quantity)
        if risk_unit > 0 and trade.quantity else None
    )
...
async def get_journal(limit: int = 50, db: AsyncSession = Depends(get_db)):
    ...
    stmt   = (
        select(Trade)
        .where(Trade.user_id == DEFAULT_USER_ID)
...
async def get_analytics(db: AsyncSession = Depends(get_db)):
    ...
    stmt   = select(Trade).where(
        ...
        Trade.result.not_in(["OPEN"]),
        Trade.result.is_not(None),
    )
    trades = (await db.execute(stmt)).scalars().all()
    ...
    if not trades:
        return {
            "trades_count": 0,
            "message":      "Aucun trade clôturé — commencez à trader pour voir vos statistiques.",
        }
...
def _ser(r: ScoreResult) -> dict:
    return {
        ...
        "reason":             r.reasons,
        "sub_scores": {
            "regime":       r.sub_scores.regime,
            "structure":    r.sub_scores.structure,
            "liquidity":    r.sub_scores.liquidity,
            "pullback":     r.sub_scores.pullback,
            "timing":       r.sub_scores.timing,
            "confirmation": r.sub_scores.confirmation,
            "risk":         r.sub_scores.risk,
            "macro":        r.sub_scores.macro,
